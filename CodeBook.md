...
The features contained within the output files 'mean_sd.txt' and 'average_activity_subject.txt' relate to the variables detailed below, which were from 'features_info.txt'. In addition to the existing variables, new variables were created and incorporated into the data set:

* activitylabel: numeric labels for activities.
...
Characters such as ()-_., were stripped out of the variable names and the names were converted to lower case in 'mean_sd.txt' and 'average_activity_subject.txt'. Prefix of the manipulated variable names with 'avg' in the table 'average_activity_subject.txt' indicates that the data represent the average of all data available for a subject and the chosen activity.
